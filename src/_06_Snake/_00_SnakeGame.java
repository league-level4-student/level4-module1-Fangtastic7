...
public class _00_SnakeGame implements ActionListener, KeyListener {
	...
	public void startGame() {
		...
		String instructions = " Your snake cannot stop moving to collect food. Try to elongate your snake by collecting food. However, try not to run into the walls or even yourself!";
		...
		switch(choice) {
		case "Expert":
			timer.setDelay(hardspeed);
			break;
		case "Moderate":
			timer.setDelay(mediumspeed);
			break;
		default:
			timer.setDelay(easyspeed);
			break;
		}
		//3. start the timer
		timer.start();
	...
	private void setFoodLocation() {
		...
		Location newlocation = new Location(random.nextInt(WIDTH), random.nextInt(HEIGHT));
